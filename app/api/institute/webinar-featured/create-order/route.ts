...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (canPayViaWallet) {
      stage = "wallet_debit";
      const idempotencyKey = buildWalletDebitIdempotencyKey("webinar_featured_orders", orderId);
      const debitResult = await admin.data
        ...
        .insert({
          ...
          updated_at: nowIso,
          metadata: {
            reason: "webinar_featured_subscription_wallet_debit",
            order_table: "webinar_featured_orders",
            order_id: orderId,
          },
        });
      ...
      const auditResult = await logInstituteWalletEvent({
        ...
        sourceTable: "webinar_featured_orders",
        sourceId: orderId,
        orderId,
        orderKind: "featured_webinar",
        ...
        metadata: {
          ...
          plan_id: String(resolvedPlan.id),
          target_type: "webinar",
          target_id: body.webinarId,
        },
      });
      if (!auditResult.ok) return NextResponse.json({ error: auditResult.error }, { status: 500 });

      const verifyResponse = await fetch(new URL("/api/institute/webinar-featured/verify", request.url), {
        method: "POST",
      ...
      if (!verifyResponse.ok) {
        return NextResponse.json({ error: verifyBody?.error ?? "Unable to activate webinar promotion from wallet." }, { status: verifyResponse.status });
      }

      await notifyInstituteAndAdmins({
        admin: admin.data,
        instituteUserId: auth.user.id,
        title: "Webinar promotion activated via wallet",
        message: "A webinar featured promotion was activated using institute wallet balance.",
        metadata: { webinarId: body.webinarId, orderId, planId: String(resolvedPlan.id), paymentMethod: "wallet" },
      });
