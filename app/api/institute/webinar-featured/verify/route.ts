...
import { expireWebinarFeaturedSubscriptionsSafe, isWebinarPromotable } from "@/lib/webinar-featured";
...
export async function POST(request: Request) {
  ...
  let { data: existingOrder } = await admin.data
    ...
    .select("id,institute_id,created_by,webinar_id,plan_id,amount,currency,duration_days,payment_status,order_status,metadata")
  ...
  if (!activation.ok) {
    await notifyReconciliationCritical({ title: "Featured activation reconcile failed", message: "Featured payment was captured but subscription activation did not complete.", category: "featured_reconciliation", priority: "critical", targetUrl: notificationLinks.adminFeaturedReconciliationUrl(), dedupeKey: `admin:featured-verify-activation-failed:${existingOrder.id}`, metadata: { routeName: "institute/webinar-featured/verify", orderId: existingOrder.id, featuredOrderId: existingOrder.id, webinarId: existingOrder.webinar_id, instituteId: instituteId, razorpayOrderId: orderId, razorpayPaymentId: paymentId, failureReason: activation.error ?? "activation_failed" } });
