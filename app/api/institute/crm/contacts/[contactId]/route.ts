...
async function scopedContact(admin: ReturnType<typeof import("@/lib/supabase/admin").getSupabaseAdmin>["data"], instituteId: string, contactId: string) {
  return admin.from("crm_contacts").select("*").eq("id", contactId).eq("owner_type", "institute").eq("owner_institute_id", instituteId).eq("is_deleted", false).maybeSingle();
}
...
export async function GET(_: Request, { params }: { params: Promise<{ contactId: string }> }) {
  ...
  const c = await scopedContact(admin, instituteId, contactId);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ contactId: string }> }) {
  ...
  const existing = await scopedContact(admin, instituteId, contactId); if (!existing.data) return NextResponse.json({ error: "Contact not found" }, { status: 404 });
