...
export async function GET(request: Request) {
  ...
  let q = admin.from("crm_contacts").select("id,full_name,email,phone,whatsapp_number,source,lifecycle_stage,priority,next_follow_up_at,last_activity_at,created_at,assigned_to,course_id,webinar_id,converted,is_archived,courses(title),webinars(title)", { count: "exact" })
    .eq("owner_type", "institute").eq("owner_institute_id", instituteId).eq("is_deleted", false);
  ...
  if (p.get("archived")) q = q.eq("is_archived", p.get("archived") === "true");
