...
export default function InstituteCourseFeaturedPage() {
      ...
      <section className="mt-6 rounded-xl border bg-white p-4">
        ...
        <div className="mt-4 space-y-3">
          {courses.map((course) => {
            ...
            const actionText = state.active ? "Extend Feature" : "Feature Course";
              ...
              <div key={course.id} className="rounded border p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    {state.scheduled ? (
                      <p className="mt-1 text-xs text-amber-700">Scheduled Next · Starts {when(state.scheduled.starts_at)} · Expires {when(state.scheduled.ends_at)}</p>
                    ) : null}
                  </div>
                  ...
                  <div className="flex w-full flex-col gap-2 lg:w-auto lg:min-w-[320px]">
                    ...
                    <button
                      ...
                      disabled={Boolean(busyCourseId) || plans.length === 0}
                    >
        ...
        <div className="mt-3 space-y-2 text-sm">
          {subscriptions.map((item) => (
            <div key={item.id} className="rounded border p-3">
              <p className="font-medium">Course ID: {item.course_id} · {item.plan_name ?? item.plan_code ?? "Plan"}</p>
              <p className="text-slate-600">Status: {item.status} · Starts: {when(item.starts_at)} · Ends: {when(item.ends_at)}</p>
        ...
        <div className="mt-3 space-y-2 text-sm">
          {orders.map((order) => (
            <div key={order.id} className="rounded border p-3">
              <p className="font-medium">Course ID: {order.course_id} · {inr(order.amount, order.currency)}</p>
