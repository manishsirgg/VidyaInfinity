...
export default async function InstituteCrmPage() {
  ...
  const [contacts, follow, acts] = await Promise.all([
    supabase.from("crm_contacts").select("id,lifecycle_stage,priority,course_id,webinar_id,converted,is_archived").eq("owner_type", "institute").eq("owner_institute_id", institute.id).eq("is_deleted", false),
    supabase.from("crm_follow_ups").select("id,due_at,status").eq("institute_id", institute.id).eq("is_deleted", false),
    supabase.from("crm_activities").select("id,title,created_at").eq("institute_id", institute.id).order("created_at", { ascending: false }).limit(8),
  ]);
