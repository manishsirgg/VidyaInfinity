...
export default async function ContactsPage({ searchParams }: { searchParams: Promise<{ q?: string; stage?: string; priority?: string }> }) {
  ...
  if (instituteError) {
    console.error("Failed to resolve institute for CRM contacts", { userId: user.id, error: instituteError.message });
  ...
  let q = dataClient.from("crm_contacts").select("id,full_name,email,phone,whatsapp_number,source,lifecycle_stage,priority,next_follow_up_at,created_at").eq("owner_type","institute").eq("owner_institute_id",institute.id).eq("is_deleted",false).eq("is_archived", false);
  ...
  if (error) {
    console.error("Failed to load CRM contacts", { instituteId: institute.id, error: error.message });
    throw new Error("Unable to load CRM contacts.");
  }
