...
export default async function EditTestPage({ params }: { params: Promise<{ testId: string }> }) {
  ...
  return <div className="space-y-4"><PsychometricAdminHeader title="Edit Psychometric Test" description="Refine form fields and scoring setup." breadcrumbs={[{ label: "Tests", href: "/admin/psychometric/tests" }, { label: test?.title ?? "Edit" }]} /><PsychometricAdminSubnav currentPath="/admin/psychometric/tests" /><TestForm initial={test ?? {}} testId={testId} /></div>;
}
