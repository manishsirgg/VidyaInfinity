...
export default async function PsychometricTestsAdminPage({ searchParams }: { searchParams?: Promise<{ q?: string; status?: string }> }) {
  ...
  return <div className="space-y-4 p-3 md:p-6"><PsychometricAdminHeader title="Psychometric Tests" description="Create and manage psychometric assessments." breadcrumbs={[{label:"Admin",href:"/admin/dashboard"},{label:"Psychometric",href:"/admin/psychometric"},{label:"Tests"}]} action={<Link href="/admin/psychometric/tests/new" className="rounded bg-brand-600 px-3 py-2 text-sm text-white">Create Psychometric Test</Link>} /><PsychometricAdminSubnav currentPath="/admin/psychometric/tests" />
