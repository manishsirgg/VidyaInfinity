...
export default function QuestionBuilderPage({ testTitle = "Psychometric Test" }: { testTitle?: string }) {
  ...
  return <div className="space-y-5 p-2 pb-10 md:p-4">
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      <PsychometricAdminCard className="space-y-3 lg:col-span-1"><h2 className="font-semibold">Question List</h2><p className="text-xs text-slate-500">Select a question to edit, duplicate, or deactivate.</p>
        {loading ? <div className="text-sm text-slate-500">Loading questions...</div> : questions.length === 0 ? <PsychometricEmptyState title="No questions yet" subtitle="Create your first question from the editor panel." /> : questions.map((q, idx) => <div key={q.id} className={`rounded-lg border p-2.5 ${selectedQuestionId === q.id ? "border-brand-600" : ""}`}><div className="flex items-center justify-between"><button className="text-left text-sm font-medium" onClick={() => setSelectedQuestionId(q.id)}>{idx + 1}. {q.question_text.slice(0, 54) || "Untitled question"}</button><span className="text-xs">#{q.sort_order}</span></div><div className="mt-1 flex flex-wrap gap-1 text-xs"><PsychometricStatusBadge label={q.question_type} tone="blue" /><PsychometricStatusBadge label={q.is_required ? "Required" : "Optional"} tone={q.is_required ? "emerald" : "slate"} /><PsychometricStatusBadge label={q.is_active ? "Active" : "Inactive"} tone={q.is_active ? "emerald" : "amber"} /></div><div className="mt-2 flex gap-2 text-xs"><button className="rounded-lg border px-2 py-1 hover:bg-slate-50" onClick={async()=>{if(!confirm("Duplicate this question?")) return; const r=await fetch(`/api/admin/psychometric/questions/${q.id}/duplicate`,{method:"POST"}); setBanner({kind:r.ok?"success":"error",message:r.ok?"Question duplicated.":"Duplicate failed."}); await load();}}>Duplicate</button><button className="rounded-lg border px-2 py-1 hover:bg-slate-50" onClick={async()=>{if(!confirm("Deactivate this question?")) return; const r=await fetch(`/api/admin/psychometric/questions/${q.id}`,{method:"DELETE"}); setBanner({kind:r.ok?"success":"error",message:r.ok?"Question deactivated.":"Deactivate failed."}); await load();}}>Deactivate</button></div></div>)}
      </PsychometricAdminCard>

      <div className="space-y-4 lg:col-span-2">
        <PsychometricAdminCard className="space-y-3"><h2 className="font-semibold">Create Question</h2><p className="text-xs text-slate-500">Add question text first, then refine details in the editor.</p>
          <input className="w-full rounded-lg border p-2.5" placeholder="question_text" value={draft.question_text} onChange={(e) => setDraft({ ...draft, question_text: e.target.value })} />
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2"><select className="rounded-lg border p-2.5" value={draft.question_type} onChange={(e) => setDraft({ ...draft, question_type: e.target.value as QuestionType })}>{QUESTION_TYPES.map((t) => <option key={t}>{t}</option>)}</select><input type="number" className="rounded-lg border p-2.5" value={draft.sort_order} onChange={(e)=>setDraft({...draft,sort_order:Number(e.target.value||1)})} /></div>
          <button disabled={saving} onClick={() => void saveQuestion("create")} className="rounded-lg bg-brand-600 px-3 py-2 text-white disabled:opacity-50">{saving ? "Saving..." : "Add question"}</button>
        </PsychometricAdminCard>

        {selectedQuestion && <PsychometricAdminCard className="space-y-3"><h2 className="font-semibold">Editor + Preview</h2><p className="text-xs text-slate-500">Fine-tune options, scoring, and visibility for the selected question.</p>
          ...
          <input className="w-full rounded-lg border p-2.5" value={selectedQuestion.question_text} onChange={(e) => updateSelected({ question_text: e.target.value })} />
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2"><select className="rounded-lg border p-2.5" value={selectedQuestion.question_type} onChange={(e)=>updateSelected({question_type:e.target.value as QuestionType})}>{QUESTION_TYPES.map((t)=><option key={t}>{t}</option>)}</select><input type="number" className="rounded-lg border p-2.5" value={selectedQuestion.weight} onChange={(e)=>updateSelected({weight:Number(e.target.value||1)})} /></div>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-3"><input type="number" className="rounded-lg border p-2.5" value={selectedQuestion.sort_order} onChange={(e)=>updateSelected({sort_order:Number(e.target.value||1)})} /><label className="text-sm"><input type="checkbox" checked={selectedQuestion.is_required} onChange={(e)=>updateSelected({is_required:e.target.checked})}/> Required</label><label className="text-sm"><input type="checkbox" checked={selectedQuestion.is_active} onChange={(e)=>updateSelected({is_active:e.target.checked})}/> Active</label></div>
          ...
          {selectedQuestion.question_type === "scale" && <div className="grid grid-cols-2 gap-2"><input className="rounded-lg border p-2.5" type="number" value={selectedQuestion.min_scale_value||1} onChange={(e)=>updateSelected({min_scale_value:Number(e.target.value||1)})} /><input className="rounded-lg border p-2.5" type="number" value={selectedQuestion.max_scale_value||5} onChange={(e)=>updateSelected({max_scale_value:Number(e.target.value||5)})} /></div>}
          <div className="rounded-lg border bg-slate-50 p-3 text-sm"><p className="mb-1 font-medium">Student Preview</p><p>{selectedQuestion.question_text || "Question text preview"}</p></div>
