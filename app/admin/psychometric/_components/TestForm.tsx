...
export default function TestForm({ initial, testId }: { initial?: Record<string, any>; testId?: string }) {
  ...
  return <div className="space-y-4">
    ...
    {testId && <div className="flex flex-wrap gap-2 text-sm"><Link href={`/admin/psychometric/tests/${testId}/questions`} className="rounded-lg border px-3 py-1.5">Manage Questions</Link><Link href={`/psychometric-tests/${effectiveSlug}`} className="rounded-lg border px-3 py-1.5">Preview Public Page</Link><Link href="/admin/psychometric/tests" className="rounded-lg border px-3 py-1.5">Back to Tests</Link></div>}

    <PsychometricAdminCard className="space-y-3"><h2 className="text-lg font-semibold">Basic Details</h2>
      <input className="w-full rounded border p-2" placeholder="Title" value={form.title || ""} onChange={(e) => setField("title", e.target.value)} />
      {errors.title && <p className="text-xs text-rose-600">{errors.title}</p>}
      <input className="w-full rounded border p-2" placeholder="Slug" value={effectiveSlug} onChange={(e) => { setManualSlug(true); setField("slug", e.target.value); }} />
      {errors.slug && <p className="text-xs text-rose-600">{errors.slug}</p>}
      <input className="w-full rounded border p-2" placeholder="Category" value={form.category || ""} onChange={(e) => setField("category", e.target.value)} />
      <textarea className="w-full rounded border p-2" placeholder="Description" value={form.description || ""} onChange={(e) => setField("description", e.target.value)} />
    </PsychometricAdminCard>

    <PsychometricAdminCard className="space-y-3"><h2 className="text-lg font-semibold">Pricing & Visibility</h2>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2"><input className="w-full rounded border p-2" type="number" placeholder="Price" value={form.price ?? 0} onChange={(e) => setField("price", Number(e.target.value))} /><input className="w-full rounded border p-2" type="number" placeholder="Duration (minutes)" value={form.duration_minutes ?? 60} onChange={(e) => setField("duration_minutes", Number(e.target.value))} /></div>
      {(errors.price || errors.duration_minutes) && <p className="text-xs text-rose-600">{errors.price || errors.duration_minutes}</p>}
      <div className="flex flex-wrap gap-4"><label className="text-sm"><input type="checkbox" checked={!!form.is_active} onChange={(e) => setField("is_active", e.target.checked)} /> Active</label><label className="text-sm"><input type="checkbox" checked={!!form.is_featured} onChange={(e) => setField("is_featured", e.target.checked)} /> Featured</label></div>
    </PsychometricAdminCard>

    <PsychometricAdminCard className="space-y-3"><h2 className="text-lg font-semibold">Student Instructions</h2><textarea className="w-full rounded border p-2" rows={4} placeholder="Instructions" value={form.instructions || ""} onChange={(e) => setField("instructions", e.target.value)} /></PsychometricAdminCard>

    <PsychometricAdminCard className="space-y-3"><h2 className="text-lg font-semibold">Scoring Configuration</h2>
      {bands.length === 0 ? <PsychometricEmptyState title="No result bands" subtitle="Add one or more score bands." /> : bands.map((b, i) => <div key={`${b.label}-${i}`} className="rounded border p-3"><div className="grid grid-cols-1 gap-2 md:grid-cols-4"><input className="rounded border p-2" placeholder="Label" value={b.label} onChange={(e)=>setField("scoring_config",{...form.scoring_config,bands:bands.map((x,j)=>j===i?{...x,label:e.target.value}:x)})}/><input className="rounded border p-2" type="number" placeholder="Min" value={b.min} onChange={(e)=>setField("scoring_config",{...form.scoring_config,bands:bands.map((x,j)=>j===i?{...x,min:Number(e.target.value)}:x)})}/><input className="rounded border p-2" type="number" placeholder="Max" value={b.max} onChange={(e)=>setField("scoring_config",{...form.scoring_config,bands:bands.map((x,j)=>j===i?{...x,max:Number(e.target.value)}:x)})}/><button className="rounded border px-3" onClick={()=>setField("scoring_config",{...form.scoring_config,bands:bands.filter((_,j)=>j!==i)})}>Remove</button></div>
      ...
      <button className="rounded border px-3 py-2" onClick={()=>setField("scoring_config",{...form.scoring_config,bands:[...bands,{label:"",min:0,max:0}]})}>Add Band</button>
      <div className="text-xs"><PsychometricStatusBadge label="Validation checks overlap and 0-100 bounds" tone="amber" /></div>
    </PsychometricAdminCard>

    <PsychometricAdminCard className="space-y-3"><h2 className="text-lg font-semibold">Advanced</h2>
    ...
    <div><button onClick={save} disabled={saving} className="rounded bg-brand-600 px-4 py-2 text-white disabled:opacity-50">{saving ? "Saving..." : "Save Test"}</button></div>
  </div>;
}
