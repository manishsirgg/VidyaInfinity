...
export default function PsychometricDiagnosticsPage() {
  ...
  return <div className="space-y-6">
    <h1 className="text-2xl font-semibold">Psychometric Diagnostics</h1>
    ...
    <section><h2 className="mb-2 text-lg font-medium">System Health</h2><div className="grid gap-3 md:grid-cols-4">{Object.entries(data?.counters ?? {}).map(([k, v]) => <div key={k} className="rounded border bg-white p-3"><p className="text-xs text-slate-500">{k}</p><p className="text-xl font-semibold">{v}</p></div>)}</div></section>
    <section><h2 className="mb-2 text-lg font-medium">Broken State Counters</h2><div className="grid gap-3 md:grid-cols-3">{Object.entries(data?.broken ?? {}).map(([k, v]) => <div key={k} className="rounded border bg-white p-3"><p className="text-xs text-slate-500">{k}</p><p className="text-xl font-semibold">{v}</p></div>)}</div></section>
    <section><h2 className="mb-2 text-lg font-medium">Schema / Helper / RLS Checklist</h2><pre className="overflow-auto rounded border bg-white p-3 text-xs">{JSON.stringify(data?.checklist ?? {}, null, 2)}</pre></section>

    <section className="space-y-4">
      <h2 className="text-lg font-medium">Recent Activity</h2>
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded border bg-white p-3"><h3 className="mb-2 font-medium">Recent Orders</h3>{(data?.recent.orders ?? []).map((o) => <div key={o.id} className="border-t py-2 text-sm"><div>{o.id}</div><div className="text-xs text-slate-600">{o.payment_status}</div>{o.attempt_id && <Link className="underline" href={`/admin/psychometric/attempts/${o.attempt_id}`}>View attempt detail</Link>}</div>)}</div>
        <div className="rounded border bg-white p-3"><h3 className="mb-2 font-medium">Recent Attempts</h3>{(data?.recent.attempts ?? []).map((a) => <div key={a.id} className="border-t py-2 text-sm"><div>{a.id}</div><div className="text-xs text-slate-600">{a.status}</div><Link className="underline" href={`/admin/psychometric/attempts/${a.id}`}>View attempt detail</Link>{a.report_id && <> · <Link className="underline" href={`/admin/psychometric/reports/${a.report_id}`}>View report detail</Link></>}</div>)}</div>
        <div className="rounded border bg-white p-3"><h3 className="mb-2 font-medium">Recent Reports</h3>{(data?.recent.reports ?? []).map((r) => <div key={r.id} className="border-t py-2 text-sm"><div>{r.id}</div><Link className="underline" href={`/admin/psychometric/reports/${r.id}`}>View report detail</Link>{r.attempt_id && <> · <Link className="underline" href={`/admin/psychometric/attempts/${r.attempt_id}`}>View attempt detail</Link></>}</div>)}</div>
        <div className="rounded border bg-white p-3"><h3 className="mb-2 font-medium">Recent Answers</h3>{(data?.recent.answers ?? []).map((a) => <div key={a.id} className="border-t py-2 text-sm"><div>{a.id}</div><div className="text-xs text-slate-600">awarded_score: {a.awarded_score ?? 0}</div>{a.attempt_id && <Link className="underline" href={`/admin/psychometric/attempts/${a.attempt_id}`}>View attempt detail</Link>}</div>)}</div>
      </div>
