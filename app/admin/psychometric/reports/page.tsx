...
export default async function ReportsPage({ searchParams }: { searchParams?: Promise<Params> }) {
  ...
  return <div className="space-y-4 bg-slate-50/60 p-3 pb-10 md:p-6">
    ...
    <PsychometricAdminCard><form className="grid gap-2 md:grid-cols-5"><input name="q" placeholder="Search by student, test, report ID" defaultValue={params.q ?? ""} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" /><select name="band" defaultValue={params.band ?? "all"} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"><option value="all">All bands</option>{resultBands.map((band) => <option key={band} value={band}>{band}</option>)}</select><select name="stale" defaultValue={params.stale ?? "all"} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"><option value="all">All freshness</option><option value="yes">Stale/Broken</option><option value="no">Healthy</option></select><select name="test" defaultValue={params.test ?? ""} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"><option value="">All tests</option>{[...testsMap.values()].map((t: any) => <option key={t.id} value={t.id}>{t.title ?? t.id}</option>)}</select><button className="rounded-xl bg-brand-600 px-3 py-2 text-sm font-medium text-white transition hover:bg-brand-700">Apply Filters</button></form></PsychometricAdminCard>
