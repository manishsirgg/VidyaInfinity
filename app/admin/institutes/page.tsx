...
export default async function Page() {
  ...
  const { data: profiles } = userIds.length
    ? await supabase
        .from("profiles")
        .select("id,full_name,email,phone,city,state,country,designation,approval_status")
        .in("id", userIds)
    : { data: [] };
        ...
        {institutes?.map((institute) => {
          ...
          return (
            <div key={institute.id} className="rounded border bg-white p-4 text-sm">
              ...
              {institute.rejection_reason ? <p className="mt-1 text-xs text-rose-600">Reason: {institute.rejection_reason}</p> : null}
              ...
              <div className="mt-2 rounded border border-slate-200 bg-slate-50 p-2">
                <p className="text-xs font-medium text-slate-700">Owner user documents</p>
                {ownerDocs.length > 0 ? (
                  <ul className="mt-1 space-y-1 text-xs">
                    {ownerDocs.map((doc) => (
                      <li key={doc.id}>
