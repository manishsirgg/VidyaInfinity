-- Ensure refunded orders always stamp payout reversal fields and never leave an original payout available.
...
begin
  if v_kind in ('course_order', 'course') then v_kind := 'course';
  elsif v_kind in ('webinar_order', 'webinar') then v_kind := 'webinar'; end if;
  if v_kind not in ('course', 'webinar') then raise exception 'Unsupported order kind for institute payout refund: %', p_order_kind; end if;
  ...
  if v_institute_id is null then raise exception 'Order % not found for kind %', p_order_id, v_kind; end if;
  ...
  if not found then
    insert into public.institute_payouts (
      institute_id, course_order_id, webinar_order_id, amount_payable, payout_amount, gross_amount, platform_fee_amount,
      payout_status, refund_amount, refund_status, refund_reference, payout_source, source_reference_id, source_reference_type,
      metadata, due_at, scheduled_at, reversed_at, created_at, updated_at
    ) values (
      v_institute_id,
      case when v_kind = 'course' then p_order_id else null end,
      case when v_kind = 'webinar' then p_order_id else null end,
      -1 * p_refund_amount, -1 * p_refund_amount, 0, 0,
      'reversed', p_refund_amount, 'refunded', v_ref, 'refund_adjustment', v_ref, 'refund_reference',
  ...
  if v_applied <= 0 then
    ...
    set refund_status = coalesce(refund_status, 'refunded'), refund_reference = coalesce(refund_reference, v_ref), reversed_at = coalesce(reversed_at, now()), updated_at = now()
  ...
  if v_payout.payout_status in ('pending', 'available') then
    ...
    for v_alloc_id, v_alloc_amount, v_req_id in
      select a.id, coalesce(a.amount, 0), a.payout_request_id from public.institute_payout_request_allocations a where a.payout_id = v_payout.id order by a.created_at desc
    loop
      ...
      if v_alloc_amount <= v_remaining then
        ...
        update public.institute_payout_request_allocations set amount = v_alloc_amount - v_remaining, updated_at = now() where id = v_alloc_id;
        v_remaining := 0;
      end if;
      update public.institute_payout_requests set amount = greatest(coalesce(amount, 0) - least(v_alloc_amount, v_applied), 0), updated_at = now() where id = v_req_id;
    end loop;
    ...
    insert into public.institute_payouts (
      ...
      v_payout.institute_id, v_payout.course_order_id, v_payout.webinar_order_id,
      -1 * v_applied, -1 * v_applied, 0, 0,
      'reversed', v_applied, 'recovery', v_ref, 'refund_adjustment', v_ref, 'refund_reference',
      jsonb_build_object('source_payout_id', v_payout.id, 'refund_reference', v_ref, 'refund_event_id', v_event_id, 'recovery_reason', 'refund_after_processed_payout'),
      now(), now(), now(), now(), now()
    ) returning id into v_adjustment_id;
    ...
    set refund_amount = v_existing_refund + v_applied,
        ...
        reversed_at = now(),
        updated_at = now(),
        metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('last_refund_reference', v_ref, 'last_refund_amount', v_applied, 'last_refund_at', now(), 'recovery_adjustment_id', v_adjustment_id)
    where id = v_payout.id;
  else
    update public.institute_payouts
    set refund_amount = v_existing_refund + v_applied,
