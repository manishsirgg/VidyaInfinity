...
export function FeaturedReconciliationPanel() {
  ...
  return <div className="mt-6 space-y-4">
    ...
    {!loading && !error && visibleRows.length > 0 ? <div className="overflow-x-auto rounded border bg-white"><table className="min-w-[1400px] w-full text-xs"><thead><tr className="bg-slate-50 text-left">{["Issue","Type","Institute","Item","Plan","Amount","Local Payment","Local Order","Subscription","Razorpay Order ID","Razorpay Payment ID","Created","Paid","Recommended Action","Actions"].map((h)=><th key={h} className="px-2 py-2">{h}</th>)}</tr></thead><tbody>{visibleRows.map((row)=><tr key={`${row.orderType}-${row.id}`} className="border-t"><td className="px-2 py-2"><span className={`rounded px-2 py-1 ${badgeClass(row.issue)}`}>{row.issue}</span></td><td className="px-2 py-2">{row.orderType}</td><td className="px-2 py-2">{row.institute_id ?? "-"}</td><td className="px-2 py-2">{row.course_id ?? row.webinar_id ?? row.id}</td><td className="px-2 py-2">{row.plan_id ?? "-"}</td><td className="px-2 py-2">₹{Number(row.amount ?? 0).toLocaleString("en-IN")}</td><td className="px-2 py-2"><span className={`rounded px-2 py-1 ${badgeClass(row.payment_status ?? "unknown")}`}>{row.payment_status ?? "unknown"}</span></td><td className="px-2 py-2">{row.order_status ?? "-"}</td><td className="px-2 py-2"><span className={`rounded px-2 py-1 ${badgeClass(row.issue.includes("paid") ? "missing subscription" : "active")}`}>{row.issue.includes("paid") ? "missing subscription" : "active"}</span></td><td className="px-2 py-2">{row.razorpay_order_id ?? "-"}</td><td className="px-2 py-2">{row.razorpay_payment_id ?? "-"}</td><td className="px-2 py-2">{new Date(row.created_at).toLocaleString()}</td><td className="px-2 py-2">{row.paid_at ? new Date(row.paid_at).toLocaleString() : "-"}</td><td className="px-2 py-2">{row.recommendedAction}</td><td className="px-2 py-2"><div className="flex flex-wrap gap-1"> <button disabled={runningActionId!==null || !row.razorpay_order_id} className="rounded bg-blue-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>runAction('/api/admin/featured-reconciliation/reconcile',{orderType:row.orderType,orderId:row.id,razorpayOrderId:row.razorpay_order_id},'Reconciled successfully',row.id)}>Reconcile</button><button disabled={runningActionId!==null} className="rounded bg-emerald-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>setModal({kind:'manual',row})}>Manual Feature</button><button disabled={runningActionId!==null} className="rounded bg-rose-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>setModal({kind:'cancel',row})}>Cancel</button><button disabled={runningActionId!==null} className="rounded bg-amber-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>setModal({kind:'extend',row})}>Extend</button><button disabled={runningActionId!==null} className="rounded bg-slate-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>setModal({kind:'details',row})}>Details</button></div></td></tr>)}</tbody></table></div> : null}
