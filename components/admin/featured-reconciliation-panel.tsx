...
export function FeaturedReconciliationPanel() {
  ...
  return <div className="mt-6 space-y-4">
    ...
    {!loading && !error && visibleRows.length > 0 ? <div className="overflow-x-auto rounded border bg-white"><table className="min-w-[1400px] w-full text-xs"><thead><tr className="bg-slate-50 text-left">{["Issue","Type","Institute","Item","Plan","Amount","Local Payment","Local Order","Subscription","Razorpay Order ID","Razorpay Payment ID","Created","Paid","Recommended Action","Actions"].map((h)=><th key={h} className="px-2 py-2">{h}</th>)}</tr></thead><tbody>{visibleRows.map((row)=>{const normalizedSubscriptionStatus=(row.subscription_status??"").toLowerCase(); const hasRealSubscription=["active","scheduled","cancelled"].includes(normalizedSubscriptionStatus); const isDuplicateIssue=row.issue === "duplicate_paid_scheduled_upgrade"; const isMissingSubscription=row.issue.includes("paid") && !hasRealSubscription; return <tr key={`${row.orderType}-${row.orderId}`} className="border-t"><td className="px-2 py-2"><span className={`rounded px-2 py-1 ${badgeClass(row.issue)}`}>{row.issue}</span></td><td className="px-2 py-2">{row.orderType}</td><td className="px-2 py-2">{row.instituteId ?? "-"}</td><td className="px-2 py-2">{row.targetId ?? "-"}</td><td className="px-2 py-2">{row.activePlanCode && row.paidPlanCode ? `${row.activePlanCode} → ${row.paidPlanCode}` : (row.plan_id ?? "-")}</td><td className="px-2 py-2">₹{Number(row.amount ?? 0).toLocaleString("en-IN")}</td><td className="px-2 py-2"><span className={`rounded px-2 py-1 ${badgeClass(row.payment_status ?? "unknown")}`}>{row.payment_status ?? "unknown"}</span></td><td className="px-2 py-2">{row.order_status ?? "-"}</td><td className="px-2 py-2"><span className={`rounded px-2 py-1 ${badgeClass(isMissingSubscription ? "missing subscription" : normalizedSubscriptionStatus || "active")}`}>{isMissingSubscription ? "missing subscription" : (row.subscription_status ?? "active")}</span></td><td className="px-2 py-2">{row.razorpayOrderId ?? "-"}</td><td className="px-2 py-2">{row.razorpayPaymentId ?? "-"}</td><td className="px-2 py-2">{new Date(row.created_at).toLocaleString()}</td><td className="px-2 py-2">{row.paid_at ? new Date(row.paid_at).toLocaleString() : "-"}</td><td className="px-2 py-2">{row.recommendedAction}</td><td className="px-2 py-2"><div className="flex flex-wrap gap-1"> <button disabled={runningActionId!==null || isDuplicateIssue || (row.issue==="paid_featured_order_missing_subscription" && !row.razorpayOrderId && row.orderType !== "institute")} className="rounded bg-blue-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>{ const payload={orderType:row.orderType,orderId:row.orderId}; console.log('[featured-reconciliation] reconcile row', row); console.log('[featured-reconciliation] reconcile payload', payload); runAction('/api/admin/featured-reconciliation/reconcile',payload,'Reconciled successfully',row.orderId); }}>Reconcile</button><button disabled={runningActionId!==null} className="rounded bg-emerald-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>setModal({kind:'manual',row})}>Manual Feature</button><button disabled={runningActionId!==null} className="rounded bg-rose-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>setModal({kind:'cancel',row})}>Cancel</button><button disabled={runningActionId!==null} className="rounded bg-amber-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>setModal({kind:'extend',row})}>Extend</button><button disabled={runningActionId!==null} className="rounded bg-slate-600 px-2 py-1 text-white disabled:opacity-50" onClick={()=>setModal({kind:'details',row})}>Details</button></div></td></tr>;})}</tbody></table></div> : null}
