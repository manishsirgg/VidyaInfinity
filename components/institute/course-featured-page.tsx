...
export function InstituteCourseFeaturedPageClient() {
  ...
  async function trackCheckoutEvent(orderId: string, event: "checkout_opened" | "checkout_dismissed" | "payment_failed", payload?: { reason?: string; paymentId?: string }) {
    await fetch("/api/institute/course-featured/payment-status", {
      method: "POST",
      ...
      <section className="mt-6 rounded-xl border bg-white p-4">
        ...
        <div className="mt-4 space-y-3">
          {courses.map((course) => {
            ...
            const isButtonDisabled = Boolean(busyCourseId) || plans.length === 0 || lowerOrEqualSelected || duplicateScheduledSamePlan;
